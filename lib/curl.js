...
/** Newman / HTTP client — không có trong curl production. */
...
function filterAndSortHeaders(headers) {
  const list = (headers || []).filter((h) => h && h.key && !h.disabled && !isFingerprintHeader(h.key));
  const rank = (key) => {
    const i = HEADER_ORDER.indexOf(String(key).toLowerCase());
    return i === -1 ? HEADER_ORDER.length : i;
  };
...
function buildCurl(request, options = {}) {
  ...
  let headers = filterAndSortHeaders(headersToList(json.header));
