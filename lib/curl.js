...
function isSecretHeader(key) {
  const k = String(key || '');
  if (/^postman-token$/i.test(k)) return false;
...
function buildCurl(request, options = {}) {
  ...
  let headers = headersToList(json.header);
  ...
  // Style gần DevTools: curl 'url' -X POST -H ... --data-urlencode ...
  ...
  for (const h of headers) {
    if (!h.key) continue;
    if (/^content-length$/i.test(h.key)) continue;
...
module.exports = { buildCurl, resolveUrl, headersToList, bodyToString, maskSecrets };
