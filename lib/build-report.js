...
const { buildCurl } = require('./curl');

const MD_PREVIEW_MAX = 2048;
  ...
  <article class="case" id="case-${index}">
    ...
    <div class="case-body">
      ...
      <div class="section">
        ...
        <p class="preview-note">Curl = header production + request-id (không Postman-Token / Cache-Control). Host/Connection do HTTP client gắn trên wire. Authorization / Cookie / password ẩn trong report; copy curl không replay được phần đã ẩn.</p>
...
function caseStatus(c) {
  ...
  if (c.failedAssertions > 0) return 'fail';
  return 'pass';
}
...
function mdFence(text) {
  ...
  const longest = (body.match(/`+/g) || []).reduce((n, s) => Math.max(n, s.length), 2);
  const ticks = '`'.repeat(longest + 1);
  return `${ticks}\n${body}\n${ticks}`;
}

function assertionErrorText(error) {
  if (!error) return '';
  if (typeof error === 'string') return error;
  return error.message || String(error);
}

function headersAlreadyMasked(headers) {
  return (headers || []).some((h) => /\[đã gửi \d+ ký tự/.test(String(h.value || '')));
}

function caseCurl(c) {
  if (c.curl) return c.curl;
  if (!c.url) return '';
  return buildCurl(
    {
      method: c.method,
      ...
      body: c.requestBody ? { mode: 'raw', raw: c.requestBody } : undefined,
    },
    { mask: !headersAlreadyMasked(c.requestHeaders) }
  );
}

function responseRelPath(c) {
  return c.responseSaved?.relativePath || c.responseFile || '';
}

function responsePreview(c) {
  if (c.responseSaved?.storage === 'inline') {
    return c.responseSaved.body || c.responseSaved.preview || '';
  }
  return c.responseSaved?.preview || '';
}

function renderMdCase(index, c) {
  const status = caseStatus(c);
  const id = `case-${index + 1}`;
  const lines = [];
  lines.push(`<a id="${id}"></a>`);
  lines.push(`### ${index + 1}. ${c.folder} — ${c.name}`);
  lines.push('');
  lines.push(`${statusEmoji(status)} **${statusLabel(status)}** · \`${c.method || '—'}\` \`${c.url || ''}\``);
  lines.push('');

  if (c.error) {
    lines.push('#### Lỗi');
    lines.push('');
    lines.push(mdFence(c.error));
    lines.push('');
  }
  ...
  if (!assertions.length) {
    lines.push('_Không có assertion_');
  } else {
    for (const a of assertions) {
      const ok = !a.error;
      const err = assertionErrorText(a.error);
      const name = a.name || a.assertion || 'assertion';
      lines.push(`- ${ok ? '✓' : '✗'} ${name}${err ? ` — ${err}` : ''}`);
    }
  }
  lines.push('');
  ...
  if (c.requestBody) {
    lines.push('- Body:');
    lines.push('');
    lines.push(mdFence(c.requestBody));
    lines.push('');
  }
  const reqHeaders = (c.requestHeaders || []).filter((h) => h && h.key);
  if (reqHeaders.length) {
    lines.push('');
    lines.push('Headers:');
    lines.push('');
    lines.push(mdFence(reqHeaders.map((h) => `${h.key}: ${h.value}`).join('\n')));
    lines.push('');
  }

  const curl = caseCurl(c);
  if (curl) {
    lines.push('cURL (header production + request-id; Authorization / Cookie / password ẩn):');
    lines.push('');
    lines.push(mdFence(curl));
    lines.push('');
  }

  const saved = c.responseSaved || {};
  const rel = responseRelPath(c);
  const preview = responsePreview(c);
  lines.push('#### Response');
  lines.push('');
  lines.push(`- Status: \`${c.responseCode ?? '—'}\``);
  lines.push(`- Time: ${c.responseTime != null ? `${c.responseTime}ms` : '—'}`);
  lines.push(`- Content-Type: \`${c.responseContentType || saved.contentType || '—'}\``);
  lines.push(`- Size: ${saved.size ?? 0} bytes`);
  if (rel) {
    lines.push(`- File: [${rel}](${rel})`);
  } else if (!preview) {
    lines.push('- _Không có response body_');
  }
  if (preview) {
    lines.push('');
    lines.push(preview.length >= MD_PREVIEW_MAX || saved.truncated ? 'Preview (đầu file):' : 'Body:');
    lines.push('');
    lines.push(mdFence(preview.slice(0, MD_PREVIEW_MAX)));
    if (preview.length >= MD_PREVIEW_MAX || saved.truncated) {
      lines.push('');
      lines.push('_… nội dung đầy đủ trong file đính kèm_');
    }
  }
  lines.push('');
  return lines.join('\n');
...
function buildReportMarkdown(meta, cases) {
  const passed = cases.filter((c) => c.failedAssertions === 0 && !c.error).length;
  const failed = cases.length - passed;
  const failedCases = cases
    .map((c, i) => ({ c, i }))
    .filter(({ c }) => caseStatus(c) !== 'pass');

  const overviewRows = cases
    .map((c, i) => {
      const status = caseStatus(c);
      return `| ${i + 1} | ${mdCell(c.folder)} | [${mdCell(c.name)}](#case-${i + 1}) | ${statusEmoji(status)} ${statusLabel(status)} | ${mdCell(c.method)} | ${c.responseCode ?? '—'} | ${c.responseTime != null ? `${c.responseTime}ms` : '—'} |`;
    })
    .join('\n');

  const failedRows = failedCases
    .map(({ c, i }) => {
      const failNames = (c.assertions || [])
        .filter((a) => a.error)
        .map((a) => a.name || a.assertion || 'assertion')
        .join('; ');
      return `| ${i + 1} | ${mdCell(c.folder)} | [${mdCell(c.name)}](#case-${i + 1}) | ${c.responseCode ?? '—'} | ${mdCell(failNames || c.error || 'FAIL')} |`;
    })
    .join('\n');

  const details = cases.map((c, i) => renderMdCase(i, c)).join('\n---\n\n');

  const parts = [
    `# ${meta.title || 'Test Report'}`,
    '',
    `${statusEmoji(failed ? 'fail' : 'pass')} **${passed} pass** / **${failed} fail** / **${cases.length} total** · ${formatDuration(meta.durationMs)}`,
    '',
    '| Field | Value |',
    '|---|---|',
    `| Thời gian | ${mdCell(meta.startedAt)} → ${mdCell(meta.finishedAt)} |`,
    `| Collection | \`${mdCell(meta.collection)}\` |`,
    `| Environment | \`${mdCell(meta.environment)}\` |`,
    `| Thư mục báo cáo | \`${mdCell(meta.reportDirName)}\` |`,
    '',
  ...
  if (failedCases.length) {
    parts.push('## Fail / Error');
    parts.push('');
    parts.push('| # | Folder | Request | Status | Assertion |');
    parts.push('|---|---|---|---|---|');
    parts.push(failedRows);
    parts.push('');
  }

  parts.push('## Tổng quan');
  parts.push('');
  parts.push('| # | Folder | Request | Kết quả | Method | Status | Time |');
  parts.push('|---|---|---|---|---|---|---|');
  parts.push(overviewRows);
  parts.push('');
  parts.push('## Chi tiết');
  parts.push('');
  parts.push(details);
...
function writeReport(reportDir, meta, cases) {
  ...
  fs.writeFileSync(mdPath, buildReportMarkdown(meta, cases), 'utf8');
...
module.exports = { writeReport, buildReportHtml, buildReportMarkdown };
